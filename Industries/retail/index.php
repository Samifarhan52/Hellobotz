<?php
$basePath = '../../';
$pageTitle = 'Retail WhatsApp & Instagram Automation | InboxWa';
$pageDescription = 'WhatsApp Business API and Instagram automation for retail: booking, support, campaigns and shared inbox with InboxWa.';
...
<section class="section page-hero hero-animated" style="padding-top:1.25rem">
  <div class="container">
    ...
    <div class="hero-visual-float reveal" style="max-width:720px;margin:2rem auto 0">
      <img src="/assets/images/industries/retail/hero.webp" alt="Retail WhatsApp automation"
...
<section class="section section-gradient-1">
  <div class="container">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:1rem;margin-top:1.5rem"><div class="card reveal" style="padding:1.15rem"><strong>Store appointment booking</strong></div><div class="card reveal" style="padding:1.15rem"><strong>Omnichannel offers (WhatsApp + IG)</strong></div><div class="card reveal" style="padding:1.15rem"><strong>Loyalty balance and renewals</strong></div><div class="card reveal" style="padding:1.15rem"><strong>Click-and-collect updates</strong></div><div class="card reveal" style="padding:1.15rem"><strong>Local store team inbox</strong></div></div>
  </div>
</section>
...
<section class="section section-dark" id="flow">
  <div class="container">
    ...
    <div class="workflow-sim" id="ws-ind-retail"><div class="ws-step">Customer reaches you (WhatsApp / Instagram)</div><div class="ws-step">Retail workflow triggers in InboxWa</div><div class="ws-step">Bot qualifies or shares updates</div><div class="ws-step">Human agent in shared inbox when needed</div><div class="ws-step">CRM stage + analytics updated</div></div>
    <div class="ws-controls">
      <button type="button" class="btn btn-sm btn-outline ws-prev" style="color:#fff;border-color:rgba(255,255,255,.45);background:transparent" data-sim="ws-ind-retail">Previous</button>
      <button type="button" class="btn btn-sm btn-primary ws-next" data-sim="ws-ind-retail">Next</button>
      <button type="button" class="btn btn-sm btn-outline ws-auto" style="color:#fff;border-color:rgba(255,255,255,.45);background:transparent" data-sim="ws-ind-retail">Auto-play</button>
    </div>
  </div>
</section>
...
<section class="section">
  <div class="container" style="max-width:800px;text-align:center">
    <h2 class="reveal">Channels that matter for Retail</h2>
...
<section class="section section-dark">
  <div class="container"><div class="section-header reveal" style="text-align:center">
    <h2 style="color:#fff">Run Retail conversations on InboxWa</h2>
    <a href="/#contact-section" class="btn btn-primary btn-lg" style="margin-top:1rem">Talk to us</a>
  </div></div>
</section>
<script>
(function(){
  var steps=document.querySelectorAll('#ws-ind-retail .ws-step');
  var i=0,timer=null;
  function show(n){ i=Math.max(0,Math.min(n,steps.length-1)); steps.forEach(function(el,idx){el.classList.toggle('on',idx<=i);}); }
  show(0);
  document.querySelectorAll('[data-sim="ws-ind-retail"].ws-next').forEach(function(b){b.onclick=function(){show(i+1);};});
  document.querySelectorAll('[data-sim="ws-ind-retail"].ws-prev').forEach(function(b){b.onclick=function(){show(i-1);};});
  document.querySelectorAll('[data-sim="ws-ind-retail"].ws-auto').forEach(function(b){
    b.onclick=function(){
