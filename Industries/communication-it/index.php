...
<section class="section page-hero hero-animated" style="padding-top:1.25rem">
  <div class="container">
    ...
    <div class="hero-visual-float reveal" style="max-width:720px;margin:2rem auto 0">
      <img src="/assets/images/industries/communication-it/hero.webp" alt="Communication & IT WhatsApp automation"
...
<section class="section section-gradient-1">
  <div class="container">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:1rem;margin-top:1.5rem"><div class="card reveal" style="padding:1.15rem"><strong>Support ticket updates on WhatsApp</strong></div><div class="card reveal" style="padding:1.15rem"><strong>Outage / maintenance notices</strong></div><div class="card reveal" style="padding:1.15rem"><strong>Demo scheduling from Instagram</strong></div><div class="card reveal" style="padding:1.15rem"><strong>Onboarding checklist bots</strong></div><div class="card reveal" style="padding:1.15rem"><strong>Partner lead routing</strong></div></div>
  </div>
</section>
...
<section class="section section-dark" id="flow">
  <div class="container">
    ...
    <div class="ws-controls">
      ...
      <button type="button" class="btn btn-sm btn-outline ws-auto" style="color:#fff;border-color:rgba(255,255,255,.45);background:transparent" data-sim="ws-ind-communication-it">Auto-play</button>
    </div>
  </div>
</section>
...
<script>
(function(){
  var steps=document.querySelectorAll('#ws-ind-communication-it .ws-step');
  var i=0,timer=null;
  function show(n){ i=Math.max(0,Math.min(n,steps.length-1)); steps.forEach(function(el,idx){el.classList.toggle('on',idx<=i);}); }
  show(0);
  document.querySelectorAll('[data-sim="ws-ind-communication-it"].ws-next').forEach(function(b){b.onclick=function(){show(i+1);};});
  document.querySelectorAll('[data-sim="ws-ind-communication-it"].ws-prev').forEach(function(b){b.onclick=function(){show(i-1);};});
  document.querySelectorAll('[data-sim="ws-ind-communication-it"].ws-auto').forEach(function(b){
    b.onclick=function(){
